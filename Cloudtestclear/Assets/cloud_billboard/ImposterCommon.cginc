...
half2 VirtualPlaneUV( half3 planeNormal, half3 planeX, half3 planeZ, half3 center, half2 uvScale, Ray rayLocal )
{
    ...
    half planeDistance = normalDotOrigin-normalDotCenter;//normaldotorigin
    planeDistance *= -1.0;
...
void ImposterVertex( inout ImposterData imp )
{
    ...
    half2 size = _ImposterSize.xx * 2.0; // * objectScale.xx; //unity_BillboardSize.xy //EQUAl to (x, x)                
    ...
    half2 grid = VectorToGrid( pivotToCameraRay );//change into uv
    half2 gridRaw = grid;
